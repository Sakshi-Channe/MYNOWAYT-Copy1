import 'package:firebase_database/ui/firebase_animated_list.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:firebase_database/firebase_database.dart';
import 'package:nowayt_copy1/User_dashboardProductBoxLayout.dart';

class dashboard_malls extends StatefulWidget {
  @override
  _dashboard_mallsState createState() => _dashboard_mallsState();
}

class _dashboard_mallsState extends State<dashboard_malls> {
  Query _mallsreference;


  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    _mallsreference=FirebaseDatabase.instance.reference().child("Malls").orderByChild("Name");

  }


  Widget _buildmallsrestaurantitem({Map mallsrestro}){

          return Container(
            margin: EdgeInsets.symmetric(vertical: 2),
            padding: EdgeInsets.all(5),
            height: 100,
            child : ListView(

              shrinkWrap: true,
              padding: const EdgeInsets.fromLTRB(0.0, 10.0, 0.0, 10.0),
              children: <Widget> [
                RaisedButton(
                  textColor: Colors.black,
                  //textColor: Colors.white,
                  color: Colors.white,
                  child: ProductBox(
                      name: mallsrestro['Name'],
                      city: mallsrestro['City'],
                      price: 1000,
                      image: "food.png"
                  ),
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => MaterialApp()),
                    );

                  },
                ),


              ],







            ),




    );
  }

  @override
  Widget build(BuildContext context) {


    return Scaffold(
        body:

        Container(
            child:FirebaseAnimatedList(query:_mallsreference, itemBuilder:(BuildContext context,DataSnapshot snapshot,
                Animation<double>animation,int index){
              Map mallsrestro=snapshot.value;
              return _buildmallsrestaurantitem(mallsrestro : mallsrestro);

            } )





        )






    );     //]



  }
}




